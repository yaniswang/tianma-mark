...
var mark = tianma.createPipe({
    ...
    main: function (request, response) {
        ...
        if(fs.existsSync(iconPath)){
            ...
            response
                .clear()
                .write('(function(doc,win){if(win.bSetIcon!==true){win.bSetIcon=true;try{var links=doc.getElementsByTagName("link"),head=doc.getElementsByTagName("head")[0];for(var i=0,l=links.length;i<l;i++){if(/^(icon|shortcut icon)$/i.test(links[i].getAttribute("rel"))){head.removeChild(links[i]);break}}var icon=document.createElement("link");icon.type="image/x-icon";icon.rel="icon";icon.href="'+urlIco+'";head.appendChild(icon)}catch(e){}}})(document,window);\r\n' + source);
